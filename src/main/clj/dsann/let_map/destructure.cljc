(ns dsann.let-map.destructure
  "Destructure function, adapted babaskha/sci (which came from clojure/script)"
  {:no-doc true}
  (:refer-clojure :exclude [destructure]))

;; this is required because every compiler has its own version in its own namespace and so you cannot assume it exists.
  ...
  (let [bents (partition 2 bindings)
    ...
    (if (every? symbol? (map first bents))
      ...
      (if-let [kwbs (seq (filter #(keyword? (first %)) bents))]
        (throw (ex-info "Unsupported binding key" {:key (ffirst kwbs)}))
